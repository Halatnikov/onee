group("math library extension", function()
	...
	test("math.isint(arg)", function()
		assert(math.isint(1)).pass("1 is an int")
		assert(math.isint(2.5)).deny("2.5 isn't an int")
	end)
	test("math.isfloat(arg)", function()
		assert(math.isfloat(1)).deny("just a negated math.isint()")
	end)
