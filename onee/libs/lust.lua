...
function lust.describe(name, fn)
  ...
  table.insert(lust.summary, {type = "describe", name = name, level = lust.level})
  fn()
...
function lust.it(name, fn)
  table.insert(lust.summary, {type = "it", name = name, level = lust.level+1})
  ...
  local label = success and 'O PASS' or 'X FAIL'
  ...
  setmetatable(assertion, {
    ...
    __call = function(t, ...)
      if paths[t.action].test then
        ...
        if not res then
		  err = err or "unknown failure"
		  msg = msg and "\n" .. indent(lust.level + 1) .. "AT: " .. msg or ""
          error(err .. msg, 2)
        end
      end
    end
