...
function newobject:CalculateSize()
	...
	self.height = (self.rows * self.cellheight) + (self.rows * (self.cellspacing * 2))
end
...
function newobject:_update_children_position()
	for k, v in ipairs(self.children) do
		...
		local csh = rs * self.cellheight + (rs * self.cellspacing * 2)

		local cw = self.cellwidth + (self.cellpadding * 2)
		local ch = self.cellheight + (self.cellspacing * 2)
...
function newobject:AddItem(object, row, col, align)
	...
	if itemautosize then
		...
		local ch = rs * self.cellheight + (self.cellspacing * 2)
		object.width = cw - (self.cellpadding * 2)
		object.height = ch - (self.cellspacing * 2)
...
--]]---------------------------------------------------------
function newobject:SetCellPadding(padding, spacing)

	self.cellpadding = padding
	self.cellspacing = spacing or padding
	return self
...
function newobject:GetCellPadding()
	...
	return self.cellpadding, self.cellspacing
